...
class TestLaggedTransformer:
    def setup_method(self) -> None:
        self._lagged_1_categorical_transformer = LaggedTransformer(column=ColumnSpecification(name='CATEGORICAL_FEATURE', column_type=ColumnType.ORDINAL), lag=1)
        self._lagged_1_transformer = LaggedTransformer(column=ColumnSpecification(name='NUMERIC_FEATURE', column_type=ColumnType.NUMERIC), lag=1)
        self._lagged_2_transformer = LaggedTransformer(column=ColumnSpecification(name='NUMERIC_FEATURE', column_type=ColumnType.NUMERIC), lag=2)

    def test_name_and_output_type(self) -> None:
        assert self._lagged_1_categorical_transformer.output_column_specification == ColumnSpecification(name='CATEGORICAL_FEATURE_lagged_1', column_type=ColumnType.ORDINAL)
        assert self._lagged_1_transformer.output_column_specification == ColumnSpecification(name='NUMERIC_FEATURE_lagged_1', column_type=ColumnType.NUMERIC)
        assert self._lagged_2_transformer.output_column_specification == ColumnSpecification(name='NUMERIC_FEATURE_lagged_2', column_type=ColumnType.NUMERIC)
