...
class LaggedTransformer(Transformer):
    def __init__(self, column: str, column_type: ColumnType, lag: int, fill_value: Any = None) -> None:
        self._column = column
    ...
    def _transform(self) -> pl.Expr:
        return pl.col(self._column).shift(self._lag, fill_value=self._fill_value)

    def _name(self, transform: pl.Expr) -> pl.Expr:
        return transform.alias(f'{self._column}_lagged_{self._lag}')
